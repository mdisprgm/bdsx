...
export class CustomCommandFactory {
    constructor(public readonly registry: CommandRegistry, public readonly name: string) {}
    overload<PARAMS extends Record<string, CommandParameterType<any> | [CommandParameterType<any>, CommandFieldOptions | boolean]>>(
        callback: (
            params: {
                [key in keyof PARAMS]: PARAMS[key] extends [infer T, infer OPTS]
                    ? OPTS extends CommandFieldOptions | boolean
                        ? OptionalCheck<T, OPTS>
                        : never
                    : PARAMS[key] extends CommandParameterType<any>
                    ? GetTypeFromParam<PARAMS[key]>
                    : never;
            },
        ...
        for (const { key, optkey, postfix, name, options } of paramInfos) {
            ...
            const dataType =
                type instanceof CommandEnum
                    ? CommandParameterDataType.ENUM
                    : type instanceof CommandSoftEnum
        ...
        const customCommandExecute = makefunc.np(
            function (this: CustomCommandImpl, origin: CommandOrigin, output: CommandOutput) {
            ...
            void_t,
            { this: CustomCommandImpl, name: `${this.name} command::execute` },
            CommandOrigin,
            CommandOutput,
        );
...
export class CustomCommandFactoryWithSignature extends CustomCommandFactory {
    constructor(registry: CommandRegistry, name: string, public signature: CommandRegistry.Signature) {
        super(registry, name);
    }
}
...
export const command = {
    ...
    register(
        ...
        flags2: CommandUsageFlag | CommandVisibilityFlag = CommandUsageFlag._Unknown,
...
const customCommandDtor = makefunc.np(
    function (delIt) {
...
bedrockServer.withLoading().then(() => {
    executeCommandOriginal = procHacker.hooking(
